...
class PLSDAVariationsViewer:
    ...
    def start(self):
        print("\n" + "=" * 56)
        print("PLS-DA SURFACE VARIATIONS VIEWER (panel-per-mesh)")
...
def main():
    print("=" * 60)
    print("--- View PLS-DA Surface Variations ---")
    print("=" * 60)

    print("\nSelect a PLS folder (e.g., 'PLS1', 'PLS2', 'PLS3')...")
    pls_dir = popup_select_directory("Select PLS Folder")
    if not pls_dir:
        print("Canceled.")
        return

    pls_name = os.path.basename(pls_dir)

    ordered = [
        (f"{pls_name}_minus3SD.vtk", os.path.join(pls_dir, f"{pls_name}_minus3SD.vtk")),
        (f"{pls_name}_minus2SD.vtk", os.path.join(pls_dir, f"{pls_name}_minus2SD.vtk")),
        (f"{pls_name}_minus1SD.vtk", os.path.join(pls_dir, f"{pls_name}_minus1SD.vtk")),
        (f"{pls_name}_Mean.vtk", os.path.join(pls_dir, f"{pls_name}_Mean.vtk")),
        (f"{pls_name}_plus1SD.vtk", os.path.join(pls_dir, f"{pls_name}_plus1SD.vtk")),
        (f"{pls_name}_plus2SD.vtk", os.path.join(pls_dir, f"{pls_name}_plus2SD.vtk")),
        (f"{pls_name}_plus3SD.vtk", os.path.join(pls_dir, f"{pls_name}_plus3SD.vtk")),
    ]
    existing = [path for _, path in ordered if path and os.path.exists(path)]

    if not existing:
        print(f"\n[ERROR] No VTK files in {pls_dir}")
        print(f"        Expected files like {pls_name}_minus3SD.vtk ... {pls_name}_plus3SD.vtk")
        return

    PLSDAVariationsViewer(existing, title=f"PLS-DA {pls_name} variations (-3SD to +3SD)")
